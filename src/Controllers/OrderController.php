    ...
    public function getOrders(Request $request, Response $response, array $args): Response
{
    ...
    try {
        ...
        foreach ($orderRows as $orderRow) {
            ...
            $contactNumber = $orderRow[2]['value'];
            $address = $orderRow[3]['value'];

            // Get items for each order, including imageUrl
            $itemsSql = 'SELECT oi.productId, p.name AS productName, oi.quantity, p.price, p.imageUrl 
                         FROM orderItems oi 
            ...
            $items = array_map(function ($itemRow) {
                return [
                    ...
                    'imageUrl' => $itemRow[4]['value'] // Add imageUrl
                ];
            }, $itemRows);
